...
import styles from "./HistoricalPerformanceChart.module.css";
...
const HistoricalPerformanceChart: React.FC = () => {
    ...
    const enrichTickerDataArray = (arr: TickerData[]): EnrichedTickerData[] => {
        return arr.map((td) => {
            const filtered = getFilteredData(td);
            // Find high DataPoint
                ...
                <div>
                    {SYMBOLS.map((symbol) => (
                        <label key={symbol.code} className={styles.sectorLabel} style={{ display: "block", marginBottom: 8 }}>
                            ...
                            />
                            <span className={selectedSymbols.includes(symbol.code) ? styles.sectorNameSelected : styles.sectorName}>{String(symbol.name)}</span>
                        </label>
                    ))}
                </div>
            </div>
